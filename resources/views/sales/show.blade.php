<x-app-layout>
    <x-slot name="header">Detail Penjualan</x-slot>
    ...
    <div class="mb-8 flex items-end justify-between">
        <div>
            ...
            <h2 class="text-3xl font-manrope font-extrabold tracking-tight text-blue-900">
                Invoice <span class="text-primary italic">#{{ $sale->invoice_number }}</span>
            </h2>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-surface-container-lowest rounded-2xl editorial-shadow p-8 space-y-5">
            <div>
                ...
                <p class="text-sm font-bold text-blue-900">{{ $sale->date->format('d F Y') }} · <svg class="inline w-3 h-3 mr-0.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>{{ $sale->created_at->format('H:i') }}</p>
            </div>
            <div>
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Kasir</p>
                <p class="text-sm font-bold text-blue-900">{{ $sale->user->name }}</p>
            </div>
            @if($sale->notes)
            <div>
        ...
        <div class="lg:col-span-2 bg-surface-container-lowest rounded-2xl editorial-shadow overflow-hidden">
            <div class="px-8 py-5 border-b border-slate-100">
                <h3 class="font-manrope font-bold text-blue-900 text-sm uppercase tracking-widest">Item Terjual</h3>
            </div>
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-surface-container-low/50">
                        <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Produk (Product)</th>
                        <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Varian (Variant)</th>
                        <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center">Qty</th>
                        <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-right">Harga (Price)</th>
    ...
    <a href="{{ route('sales.index') }}" class="text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-widest">
