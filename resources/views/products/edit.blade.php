<x-app-layout>
    ...
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-manrope font-extrabold tracking-tight text-blue-900">
            Edit <span class="text-primary italic">{{ $product->name }}</span>
        </h2>
    </div>
        ...
        <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-5" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="purchase_price" id="purchase_price" value="{{ old('purchase_price', $lastCost ?? '') }}" min="0"
            ...
            <div class="flex items-center gap-4 pt-2">
                ...
                <a href="{{ route('products.show', $product) }}" class="text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-widest">Batal</a>
            </div>
        </form>
    </div>
    ...
    <script>
        function calcSellPrice() {
            ...
            if (beli > 0 && margin > 0) {
                ...
                document.querySelector('#price').value = jual;
            } else {
        ...
        function calcMarginFromSell() {
            const beli = parseInt(document.getElementById('purchase_price').value) || 0;
            const jual = parseInt(document.getElementById('price').value) || 0;
            const pctEl = document.getElementById('profit_pct');
            const hint = document.getElementById('profit_hint');
            const result = document.getElementById('profit_result');
            if (beli > 0 && jual > 0) {
                const pct = Math.round((jual - beli) / beli * 1000) / 10;
                pctEl.textContent = pct;
                document.getElementById('calc_margin').value = pct;
                result.textContent = jual.toLocaleString('id-ID');
                hint.style.display = 'inline';
            } else {
                hint.style.display = 'none';
