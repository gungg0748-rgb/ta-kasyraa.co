<x-app-layout>
    ...
    <div class="mb-8 flex items-end justify-between">
        <div>
            ...
            <h2 class="text-3xl font-manrope font-extrabold tracking-tight text-blue-900">
                Riwayat <span class="text-primary italic">Pembelian</span>
            </h2>
        </div>
    ...
    <form method="GET" class="mb-6 flex gap-3 flex-wrap items-end">
        ...
        @if(request()->hasAny(['supplier','from','to']))
            <a href="{{ route('purchases.index') }}" class="px-5 py-2.5 bg-surface-container text-slate-500 rounded-xl text-sm font-bold hover:bg-surface-container-high transition-colors">Reset</a>
        @endif
    </form>
    ...
    <div class="bg-surface-container-lowest rounded-2xl editorial-shadow overflow-hidden">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100/50">
                ...
                <tr class="hover:bg-blue-50/30 even:bg-slate-50/40 transition-colors">
                    <td class="px-6 py-4 text-xs font-mono font-bold text-slate-600"><div class="max-w-[160px] truncate">{{ $purchase->invoice_number }}</div></td>
                    <td class="px-6 py-4">
                        <p class="text-sm text-slate-600">{{ $purchase->date->format('d M Y') }}</p>
                        <p class="text-xs text-slate-400 mt-0.5"><svg class="inline w-3 h-3 mr-0.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>{{ $purchase->created_at->format('H:i') }}</p>
                    </td>
                    <td class="px-6 py-4 font-manrope font-bold text-blue-900 text-sm">{{ $purchase->supplier->name }}</td>
                    <td class="px-6 py-4 text-sm text-slate-500">{{ $purchase->user->name }}</td>
                ...
                <tr>
                    <td colspan="6" class="px-8 py-16 text-center">
                        <span class="material-symbols-outlined text-slate-200 text-5xl block mb-3">shopping_cart</span>
                        <p class="text-slate-400 text-sm">Belum ada transaksi pembelian.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="px-6 py-4 bg-surface-container-low/30 border-t border-slate-100/50 flex items-center justify-between">
            <span class="text-xs text-slate-400">{{ $purchases->total() }} transaksi</span>
            {{ $purchases->links() }}
        </div>
    </div>
</x-app-layout>
